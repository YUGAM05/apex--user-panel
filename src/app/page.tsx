...
const slides = [
    {
        ...
        icon: <MapPin className="w-12 h-12 text-white" strokeWidth={1.5} />,
        sideText: "15 min",
...
function RotatingBanner() {
    ...
    return (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-4 xl:px-8">
            <div className={`rounded-[3rem] p-8 md:p-16 flex flex-col md:flex-row items-center justify-between relative overflow-hidden transition-colors duration-700 shadow-2xl ${s.bgColor}`}>
                {/* Content */}
                <div className="relative z-10 max-w-2xl w-full">
                    <span className="inline-block px-4 py-1.5 mb-6 text-sm font-bold text-white bg-white/20 rounded-full backdrop-blur-sm">
                        {s.tag}
                    </span>
                    <h2 className="text-4xl md:text-5xl font-bold text-white mb-6 leading-[1.15]">
                        {s.title}
                    </h2>
                    <p className="text-white/90 text-lg md:text-xl mb-10 font-medium max-w-xl leading-relaxed">
                        {s.subtext}
                    </p>
                    <Link href={s.buttonLink} className="bg-white text-gray-900 px-8 py-3.5 rounded-xl font-bold hover:bg-gray-50 transition-colors inline-block shadow-sm">
                        {s.buttonText}
                    </Link>
                </div>

                {/* Right Visual Side */}
                <div className="relative z-10 mt-12 md:mt-0 flex flex-col items-center justify-center shrink-0 md:mr-10">
                    <div className={`w-40 h-40 md:w-48 md:h-48 rounded-full ${s.circleBg} flex items-center justify-center mb-6 transition-colors duration-700`}>
                        {s.icon}
                    </div>
                    <div className="text-center">
                        <div className="text-3xl md:text-4xl font-bold text-white mb-1 tracking-tight">{s.sideText}</div>
                        <div className="text-white/80 font-medium">{s.sideSubText}</div>
                    </div>
                </div>
